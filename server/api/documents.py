...
from server.repositories.document import DocumentRepository
...
async def upload_document(
    ...
    if len(chunks) > 200:
        logger.warning(f"Document has {len(chunks)} chunks ΓÇö limiting to 200 for free tier")
        chunks = chunks[:200]
    ...
    # Γ£à Run FAISS indexing in background so upload returns immediately
    ...
    return {
        "message": "File uploaded successfully. Indexing in background ΓÇö ready in a few seconds.",
        "document": {
            "id": doc_id_str,
            "original_filename": file.filename,
            "file_type": file_type,
